...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        control_residuals: Optional[Sequence[torch.Tensor]] = None,
        ...
        if control_residuals is not None:
            if len(control_residuals) != self.config.n_layers:
                raise ValueError(
                    f"control_residuals length {len(control_residuals)} != "
                    f"n_layers {self.config.n_layers}")
            if control_mask is not None and control_mask.dim() == 2:
                control_mask = control_mask.unsqueeze(-1)

        def add_control_residual(hidden: torch.Tensor,
                                 layer_idx: int) -> torch.Tensor:
            if control_residuals is None:
                return hidden
            residual = control_residuals[layer_idx].to(dtype=hidden.dtype,
                                                       device=hidden.device)
            if control_mask is not None:
                residual = residual * control_mask.to(dtype=hidden.dtype,
                                                       device=hidden.device)
            return hidden + control_scale * residual
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if ((self.activation_checkpointing_strategy
                    ...
                    x, cache = block(x,
                                     ...
                                     layer_past=layer_past,
                                     use_cache=use_cache)
                x = add_control_residual(x, block_idx)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
        else:
            for group_idx, block_group in enumerate(
                ...
                x, cache = block_group(x,
                                       ...
                                       use_cache=use_cache)
                last_layer_idx = min(
                    (group_idx + 1) * self.config.block_group_size - 1,
                    self.config.n_layers - 1)
                x = add_control_residual(x, last_layer_idx)
                if attn_key_values is not None:
                    assert cache is not None
